...
export default function ResourceTable({ rows, caption, onClear }: { rows: Resource[]; caption?: string; onClear?: () => void }) {
  ...
  const exportRows = filtered.map((r) => ({
    ...
    Band: r.band, PrimarySkill: r.primarySkill, SecondarySkill: r.secondarySkill, Experience: r.experience,
    CurrentProject: r.currentProject, AllocationPct: r.allocationPct, Availability: r.availabilityStatus,
  }));
  ...
  return (
    <div>
      ...
      <div className="rounded-xl border border-slate-200 overflow-hidden bg-white">
        <Table>
          <THead><TR>
            <TH>Name</TH><TH>Business Unit</TH><TH>Dept</TH><TH>Band</TH>
            <TH>Primary Skill</TH><TH>Secondary</TH><TH>Exp</TH><TH>Project</TH><TH>Alloc %</TH><TH>Availability</TH>
          </TR></THead>
          <tbody>
            {filtered.map((r) => (
              <TR key={r.employeeCode}>
                ...
                <TD>{r.businessUnit}</TD><TD>{r.department}</TD><TD>{r.band}</TD>
