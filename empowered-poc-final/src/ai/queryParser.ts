...
export function parseQuery(q: string): FinderQuery {
  ...
  const bandMatch = text.match(/band\s*(\d)/);
  const band = bandMatch ? `Band ${bandMatch[1]}` : undefined;
  ...
  return { skills: Array.from(skills), minExperience, band, department, certification, availabilityWindowDays };
}
